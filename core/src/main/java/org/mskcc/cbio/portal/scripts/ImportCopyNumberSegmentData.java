...
public class ImportCopyNumberSegmentData extends ConsoleRunnable {
    ...
    private void importData(File file, int cancerStudyId) throws IOException, DaoException {
        ...
        try {
            ...
            while ((line=buf.readLine()) != null) {
                ...
                cns.setSegId(++segId);
                if (strs.length >= 19) { // FACETS
                    cns.setSeg(parseInt(strs[6]));
                    cns.setNHet(parseInt(strs[7]));
                    cns.setCnlrMedian(parseDouble(strs[8]));
                    cns.setMafR(parseDouble(strs[9]));
                    cns.setSegCluster(parseInt(strs[10]));
                    cns.setCnlrMedianCluster(parseDouble(strs[11]));
                    cns.setMafRCluster(parseDouble(strs[12]));
                    cns.setCellularFraction(parseDouble(strs[13]));
                    cns.setTotalCopyNumber(parseInt(strs[14]));
                    cns.setMinorCopyNumber(parseInt(strs[15]));
                    cns.setCellularFractionEm(parseDouble(strs[16]));
                    cns.setTotalCopyNumberEm(parseInt(strs[17]));
                    cns.setMinorCopyNumberEm(parseInt(strs[18]));
                }
    ...
    // TODO maybe use core/src/main/java/org/mskcc/cbio/maf/TabDelimitedFileUtil.java
